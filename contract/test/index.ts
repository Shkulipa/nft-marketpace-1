...
describe("NFTMarket", () => {
  ...
  before(async () => {
    ...
    const HFTMarket = await ethers.getContractFactory("NFTMarket");
    nftMarket = await HFTMarket.deploy();
    await nftMarket.deployed();
    signers = await ethers.getSigners();
  });

  const createNFT = async (tokenURI: string) => {
    const transaction = await nftMarket.createdNFT(tokenURI);
  ...
  const createAndListNFT = async (price: number) => {
    const tokenID = await createNFT("some token url");
  ...
  describe("createNFT", () => {
    it("Should do create an NFT with correct owner and tokenURI", async () => {
  ...
  describe("listNFT", () => {
    ...
    it("should revert of not called by the owner", async () => {
      const tokenID = await createNFT(tokenURI);
  ...
  describe("buyNFT", () => {
    ...
    it("should transfer ownership to the buyer and send the price to the seller", async () => {
      ...
      const initContractBalance = await nftMarket.provider.getBalance(nftMarket.address);
      ...
      // 5% of the was kept in the contract balance
      const newContractBalance = await nftMarket.provider.getBalance(
        nftMarket.address
      );
      const contractBalanceOff = newContractBalance.sub(initContractBalance)
      expect(contractBalanceOff).to.equal(fee);

      // NFT ownership was transferred to the buyer
      const ownerAddress = await nftMarket.ownerOf(tokenID);
      expect(ownerAddress).to.equal(signers[1].address);

      // NFTTransfer event has the correct agruments
      const args = receipt.events[2].args;
      expect(args.tokenID).to.equal(tokenID);
